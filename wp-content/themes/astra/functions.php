...
function mostrar_lista_vinos() {
    // Realizar la solicitud a la API
    $response = wp_remote_get('https://api.sampleapis.com/wines/reds');
    ...
    if (is_array($response) && !is_wp_error($response)) {
        ...
        if ($data) {
            ...
            // Iterar sobre cada vino y construir la lista
            foreach ($data as $wine) {
                $output .= '<li>';
                $output .= '<h3>' . $wine->wine . '</h3>';
                $output .= '<p>Winery: ' . $wine->winery . '</p>';
                $output .= '<p>Rating: ' . $wine->rating->average . ' (' . $wine->rating->reviews . ')</p>';
                $output .= '<p>Location: ' . $wine->location . '</p>';
                $output .= '<img src="' . $wine->image . '" alt="' . $wine->wine . '">';
                $output .= '</li>';
            }
            ...
            return 'No se pudo obtener la lista de vinos.';
        }
    } else {
        return 'Error al conectar con la API.';
    }
}
add_shortcode('lista_vinos', 'mostrar_lista_vinos');
